...
def partial_label_loss(raw_outputs, partial_Y,  args, mask=None, smooth=-1, pos_coef=1.0, neg_coef=0.0):
    ...
    onezero = torch.zeros(sm_outputs.shape[0], sm_outputs.shape[1], device=cfg['device'])
    ...
    if mask is not None:
        ...
        mask = mask.unsqueeze(1).expand_as(onezero).to(cfg['device'])
    else:
        mask = torch.ones(sm_outputs.shape[0], sm_outputs.shape[1], device=cfg['device'])
