...
from utils_D import lr_cosine_policy, lr_policy, beta_policy, mom_cosine_policy, clip, denormalize, create_folder
...
class DeepInversionClass(object):
    ...
    def get_images(self, net_student=None, targets=None):
        ...
        if targets is None:
            ...
            targets = torch.LongTensor([random.randint(0, 999) for _ in range(self.bs)]).to(cfg['device'])
            if not self.random_label:
                ...
                targets = torch.LongTensor(targets * (int(self.bs / len(targets)))).to(cfg['device'])

        img_original = self.image_resolution
        targets = targets.to(cfg['device'])
        data_type = torch.half if use_fp16 else torch.float
        inputs = torch.randn((self.bs, 3, img_original, img_original), requires_grad=True, device=cfg['device'],
                             dtype=data_type)
        ...
        for lr_it, lower_res in enumerate([2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                outputs,_ = net_teacher(inputs_jit)
    ...
    def generate_batch(self, net_student=None, targets=None):
        ...
        if targets is not None:
            # targets = torch.from_numpy(np.array(targets).squeeze()).to(cfg['device'])
            if use_fp16:
                targets = targets.half()

        gen_images = self.get_images(net_student=net_student, targets=targets)
