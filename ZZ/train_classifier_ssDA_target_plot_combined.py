...
def runExperiment():
    ...
    # combined_dataset = ConcatDataset([dataset_unsup[0],dataset_unsup[1], dataset_unsup[2]])
    # list111 = [k for k in client_dataset_unsup.keys()]
    # print(list111)
    # exit()
    combined_dataset = ConcatDataset([client_dataset_unsup[k]['test'] for k in client_dataset_unsup.keys()])
    # print(combined_dataset)
    # exit()
    # #combined dataloaded
    # data_loader_unsup_comb = make_data_loader_DA(combined_dataset, 'global')
    data_loader_unsup_comb = torch.utils.data.DataLoader(combined_dataset, batch_size=32, shuffle=True)
    ...
    for epoch in range(last_epoch, cfg['global']['num_epochs'] + 1):
        ...
        if True:
            ...
            test_DA(data_loader_unsup_comb, test_model, metric, logger, epoch=0,domain=domain)
            # create loss function
            criterion = torch.nn.CrossEntropyLoss()
            
            num_batches = 20
            limited_loader = islice(data_loader_unsup_comb, num_batches)
            
            # create the hessian computation module
            hessian_comp = hessian(test_model, criterion, dataloader=data_loader_unsup_comb, cuda=True)
